...
export default function ClientesForm() {
  ...
  //Usamos lazy initializer (passadndo valor inicial da variavel
  // de um estado por meio de uma função) para fazer com que os valores
  // sejam atribuidos apenas uma vez, no carregamento (montagem) do componente
  
  const [state, setState] = React.useState(() => ({
    cliente: {uf: ''},   // Iniciado em uf: SP para conseguir puxar uf em editar clientes
    errors: {},
    isFormValid: false,
    isSnackOpen: false,
    snackMessage: '',
    ...
    sendBtnLabel: 'Enviar',
    isDialogOpen: false,
    pageTitle: 'Cadastrar novo cliente' //armazenar a rota de onde o cliente está vindo
  }))

  const { cliente, errors, isFormValid, isSnackOpen, snackMessage, 
    isServerError, sendBtnLabel, isDialogOpen, pageTitle } = state

    //React.useEffect() com vetor de dependências vazio é executado apenas na
    // fase de carregamento (montagem) do componente
    React.useLayoutEffect(() => {
      ...
      if(params.id){
        ...
        //1) Mudar o título da pagina
        //Entra no modo de edição de um registro ja existente

        axios.get(`https://api.faustocintra.com.br/clientes/${params.id}`) //busca os dados do cliente específico
        .then(
          // Carregar os dados recebidos dentro da variavel de estado "cliente"
          response => {
            setState({
              ...state,
              cliente: response.data, //conteudo dos dados do cliente ja vem espelhado
        ...
        .catch(
          error => {
            //Avre o snackbar para exibir o erro
            setState({
              ...state, 
              isSnackOpen: true,
  ...
  function formValidate(fields) {
    ...
    //console.log('data_nacimento:', fields.data_nascimento)

    //Se o campo "data-nascimento" foir uma string, convertemos para um objeto
  ...
  function saveData() {
    ...
    //Se o id estiver no link, ou seja, vamos editar o cliente, chamando PUT
    if(params.id){
