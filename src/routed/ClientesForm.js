...
export default function ClientesForm() {
  ...
  // usamos lazy initializer (passando o valor inicial da variável de estado por meio de uma função) para fazer com que os valores sejam atribuídos apenas uma vez, no carregamento (montagem) do componente
  const [state, setState] = React.useState( () => ({
    cliente: {uf: ''},   // Objeto vazio
    errors: {},
    isFormValid: false,
    isSnackOpen: false,
    snackMessage: '',
  ...
  React.useEffect(() => {
    ...
    if(params.id){
      ...
      .catch(
        error => {
          // Abre a snackbar para exibir o erro
          setState({
            ...state,
            isSnackOpen: true,
  ...
  function saveData() {
    ...
    if(params.id) {
      // Chamada de API para alteração de registro existente (axios.PUT)
