...
export default function KarangosList() {
  ...
  const columns = [
    ...
    { 
      ...
      headerAlign: 'center',
      align: 'center',
      valueFormatter: data =>  Number(data.value).toLocaleString('pt-br', { style: 'currency', currency: 'BRL' })
    },
    {
      field: 'editar',
      headerName: 'Editar',
      ...
      renderCell: data => (
        <IconButton 
          aria-label="Editar"
          onClick={() => history.push(`/karangos/${data.id}`)}
        >
          <EditIcon />
        </IconButton>
      )
    },
    {
      ...
      renderCell: data => (
        <IconButton 
          aria-label="Excluir"
          onClick={() => {console.log({data}); handleDeleteClick(data.id)}}
        >
